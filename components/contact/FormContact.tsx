...
const FormContact: React.FC<FormContactProps> = () => {
    ...
    switch (fieldName) {
      case "email":
        if (value.length < 1 || value.length > 40) {
          setFormErrors({
            ...formErrors,
            emailError: "L'email doit contenir entre 1 et 40 caractères.",
          });
      ...
      case "title":
        if (value.length < 1 || value.length > 30) {
          setFormErrors({
            ...formErrors,
            titleError: "Le titre doit contenir entre 1 et 30 caractères.",
          });
        } else {
      ...
      case "message":
        if (value.length < 1 || value.length > 255) {
          setFormErrors({
            ...formErrors,
            messageError: "Le message doit contenir entre 1 et 255 caractères.",
          });
        } else {
  ...
  return (
    <>
      <form
        className="flex flex-col w-full md:min-w-[500px] border-2 border-slate-300 bg-foreground rounded-md p-6 gap-6"
      ...
      >
        <div className="w-full">
          ...
          {formErrors.emailError && (
            <small className="text-red-500">{formErrors.emailError}</small>
          )}
        </div>
        ...
        <div className="w-full">
          ...
          {formErrors.titleError && (
            <small className="text-red-500">{formErrors.titleError}</small>
          )}
        </div>
        ...
        <div className="w-full">
          ...
          {formErrors.messageError && (
            <small className="text-red-500">{formErrors.messageError}</small>
          )}
        </div>
