...
export default function FoodConsumptionManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        closeModal();
        toast.success("Rapport de consommation bien ajouté.");
      } else {
        setError(
          data.message ||
            "Erreur lors de l'ajout de la consommation de nourriture."
        );
      }
    } catch (error: any) {
    ...
    <main className="w-full flex flex-col justify-center items-center py-6">
      <Loading loading={loading}>
        <h1 className="sm:text-3xl text-2xl mb-4 font-caption font-bold">
          Rapport de nourriture par animal
        </h1>
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
            <div className="bg-foreground p-6 rounded shadow-md md:w-1/2 text-secondary">
              <div className="flex w-full justify-between mb-6">
                <h1 className="w-full sm:text-3xl text-2xl font-bold">
                  Ajouter un rapport
                </h1>
                <button
                  onClick={closeModal}
              ...
              <form onSubmit={handleSubmit} className="text-secondary">
                ...
                <div>
                  <button
                    type="submit"
                    className="w-full bg-muted hover:bg-background text-white p-2 rounded mt-6 mb-4"
                  >
                    Créer
                  </button>
                </div>
