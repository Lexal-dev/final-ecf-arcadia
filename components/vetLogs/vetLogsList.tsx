...
export default function VetLogsList() {
  ...
  const [selectedHabitatName, setSelectedHabitatName] = useState<string | null>(
    null
  );
  ...
  const fetchAnimal = async (additionalParam: string | number) => {
    ...
    try {
      const response = await fetch(
        `/api/animals/read?additionalParam=${encodeURIComponent(
  ...
  const handleCreateVetLog = (formData: VetLog) => {
  ...
  const handleDeleteVetLog = async (id: number) => {
    try {
      ...
      if (data.success) {
        setVetLogs((prevVetLogs) =>
          prevVetLogs.filter((vetLog) => vetLog.id !== id)
        );
  ...
  useEffect(() => {
    setChoiceHabitat(
      selectedHabitatName === "" || selectedHabitatName === null
    );
  }, [selectedHabitatName]);

  const filteredAnimals = selectedHabitatName
    ? animals.filter(
        (animal) => animal.habitatId.toString() === selectedHabitatName
      )
    : animals;
  ...
  return (
    <main className="w-full flex flex-col items-center">
      <Loading loading={loading}>
        <h1 className="sm:text-3xl text-2xl mb-4 font-caption font-bold">
        ...
        <div className="flex items-center items-center gap-3">
          ...
          {choiceHabitat ? (
            ...
            <p className="sm:text-xl text-lg font-bold mb-4">
              {" "}
              [{selectedHabitatName}]:
            </p>
          )}
        ...
        <div className="overflow-x-auto w-full flex flex-col items-center">
          <table className="w-full md:w-2/3">
            <thead className="bg-muted-foreground">
              <tr>
                <th className="border border-background px-4 py-2 text-left">
                  Nom de l&apos;animal
                </th>
                <th className="border border-background px-4 py-2 text-left">
                  Race
            ...
            <tbody>
              {filteredAnimals.map((animal) => (
                <tr
                  key={animal.id}
                  className="w-full border border-background bg-foreground hover:bg-opacity-50 text-secondary hover:bg-muted hover:text-white"
            ...
            <div className="flex flex-col bg-foreground rounded-lg p-4 w-full md:w-2/3 text-secondary max-h-[80vh] overflow-y-auto">
              <ul>
                <div className="flex justify-between w-full">
                  {showCreateForm && (
                    <button
                      className="hover:text-yellow-600 text-xl hover:text-bold w-[100px]"
                  ...
                  <button
                    ...
                    className="w-full text-end text-red-600 hover:text-red-700 text-xl hover:text-bold"
                  >
                    Fermer
                  </button>
                </div>
                    ...
                    {showCreateForm ? (
                      <div className="w-full flex justify-center">
                        <FormCreate
                          ...
                          onCreate={handleCreateVetLog}
                        />
                      </div>
                    ) : (
                      <div className="w-full flex justify-center mb-6">
                        <button
                          className="hover:text-green-600 text-xl hover:text-bold "
                ...
                {filteredVetLogs.map((vetLog) => (
                  <li key={vetLog.id} className="flex mb-2 border-b-2 pb-2">
                    <div className="w-2/3">
                      ...
                      <p className="">
                        Nourriture donnée : {vetLog.foodOffered}
                      </p>
                    ...
                    <div className="w-1/3 flex items-center justify-center">
                      <MdEdit
                        className="hover:text-yellow-600 cursor-pointer text-3xl text-yellow-500"
                        onClick={() => {
                          setUpdateModal(true);
                          setUpdatedVetLog(vetLog);
                        }}
                      />
                      <MdDelete
                        className="hover:text-red-600 cursor-pointer text-3xl ml-2 text-red-500"
                        onClick={() => handleDeleteVetLog(vetLog.id)}
                      />
                    </div>
                  </li>
                ))}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center px-1">
                      <div className="bg-foreground p-6 rounded shadow-md w-full md:w-2/3">
                        ...
                        <div className="mb-4 text-secondary">
                          ...
                          <p>
                            Date : {formatDateTime(updatedVetLog?.createdAt)}
                          </p>
                        </div>
                        <FormUpdate
                          onClose={onClose}
                          vetLog={updatedVetLog}
                          onUpdate={handleUpdateVetLog}
                        />
                      </div>
                    </div>
                  </div>
