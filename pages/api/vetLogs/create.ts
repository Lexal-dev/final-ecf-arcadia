...
export default async function handler(
  ...
  if (req.method === "POST") {
    try {
      ...
      if (
        !token ||
        (!validateRoleAccess("ADMIN", token) &&
          !validateRoleAccess("VETERINARIAN", token))
      ) {
        return res
          .status(403)
          .json({
            success: false,
            message: "Access denied. Admins and employees only.",
          });
      }
      // Verify rate
      const ip =
      ...
      if (!isValidString(animalState, 3, 100)) {
        return res
          ...
          .json({
            success: false,
            message:
              "le status de l'animal doit être compris entre 3 et 100 caractére.",
          });
      }
      if (!isValidString(foodOffered, 3, 50)) {
        return res
          ...
          .json({
            success: false,
            message:
              "le nom de la nourriture doit être compris entre 3 et 50 caractére.",
          });
      }
      ...
      if (!isValidPositiveNumber(foodWeight)) {
        return res
          ...
          .json({
            success: false,
            message:
              "Le nombre doit être au-dessus de 0 et être un nombre entier",
          });
      }
      ...
      res
        ...
        .json({
          success: true,
          message: "VetLog created successfully.",
          vetLog: newVetLog,
        });
    } catch (error) {
      ...
      res
        ...
        .json({
          success: false,
          message: "Failed to create vetLog.",
          error: String(error),
        });
    }
  } else {
