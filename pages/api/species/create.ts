...
export default async function createSpecie(
  req: NextApiRequest,
  res: NextApiResponse
) {
  if (req.method === "POST") {
    ...
    try {
      ...
      if (!name) {
        res
          .status(400)
          .json({ success: false, message: "The species name is required." });
        return;
      }
      if (!isValidString(name, 3, 30)) {
        return res
          ...
          .json({
            success: false,
            message:
              "le nom de l'espèce doit être compris entre 3 et 50 caractére.",
          });
      }
